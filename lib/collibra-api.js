  ...
  return new Promise((resolve, reject) => {
    getSessionToken(ellipsis).then(token => {
      resolve({
        ...
        domainToRestrictToParams: domainToRestrictToParams,
        allRoles: allRoles,
      ...
      function apiCall(endpoint, options, resultFn, apiBaseUrlOverride) {
        ...
        const mergedHeaders = Object.assign({}, defaultArgs.headers, options.headers);
        const args = Object.assign({}, defaultArgs, options, { headers: mergedHeaders });
      ...
      function findComment(commentId) {
        return new Promise((resolve, reject) => {
          apiCall(
            `comments/${commentId}`, 
            { }, 
            handleResponse(res => resolve(res), reject)
          );
        });
      }
      
      function addComment(assetId, commentText, parentId) {
        return new Promise((resolve, reject) => {
          const data = {
            content: commentText,
            "baseResource": {
      ...
      function domainIdToRestrictToParams(dontLimitDomain) {
        return new Promise((resolve, reject) => {
          const domainId = ellipsis.env.COLLIBRA_RESTRICT_TO_DOMAIN_ID;
          if (isCannedUser(token.username) && domainId && !dontLimitDomain) {
      ...
      function domainToRestrictToParams() {
        const domainId = ellipsis.env.COLLIBRA_RESTRICT_TO_DOMAIN_ID;
        if (domainId) {
      ...
      function matchingAssets(searchQuery, options) {
        return new Promise((resolve, reject) => {
          domainIdToRestrictToParams(options.dontLimitDomain).then(params => {
            const paramsToUse = Object.assign({}, params, { 
              typeId: options.assetTypeId, 
        ...
        return new Promise((resolve, reject) => {
          definitionAttributeTypeId().then( typeId => {
            ...
            apiCall(
              ...
              handleResponse(res => resolve(res.results.filter(ea => ea.type.name === 'Definition')), reject)
            );
          });
        });
      }
