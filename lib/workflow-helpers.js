    ...
    return new Promise((resolve, reject) => {
      ...
      return storageApi.query({query: query, variables: vars }).then(res => {
        if (res.data) {
          ...
          ellipsis.error(`Error checking task started: ${JSON.stringify(res.errors)}`)
        }
      });
    });
  }
    ...
    `;
      ...
      return storageApi.query({query: mutation, variables: vars }).then(res => {
        if (res.data) {
          ...
          ellipsis.error(`Error created task started: ${JSON.stringify(res.errors)}`)
        }
      });
    });
  }
