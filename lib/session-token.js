...
return ellipsis => {
  return new Promise((resolve, reject) => {
    getSavedToken(ellipsis).then(savedToken => {
      if (savedToken) {
        isTokenValid(savedToken).then(isValid => {
          if (isValid) {
            resolve(savedToken);
          } else {
            getNewToken(ellipsis).then(resolve);
          }                   
        });
      } else {
        getNewToken(ellipsis).then(resolve);
  ...
  return new Promise((resolve, reject) => {
    ...
    request.post({
      ...
      if (err || res.statusCode != 200) {
        ...
        saveToken(token, ellipsis).then(res => resolve(token));
      }
    })
  });
}
...
function getSavedToken(ellipsis) {
  ...
  return storageApi.query({query: query}).then(res => {
    const saved = res.data.sessionTokenList[0];
    return saved ? saved.token : null;
  });
}

function saveToken(token, ellipsis) {
  const storageApi = new EllipsisApi(ellipsis).storage;

  const mutation = `
    mutation UpdateSessionToken($sessionToken: SessionTokenInput!) {
  ...
  const vars = { sessionToken: { token: token } };
